...
class NeuralPosterior(ABC):
    ...
    def map(
        ...
        try:
            ...
            while iter_ < num_iter:
                ...
                if iter_ % save_best_every == 0 or iter_ == num_iter - 1:
                    ...
                    best_log_prob = self.log_prob(
                        tf_inv(best_theta), x=x, **log_prob_kwargs
                    )

                print(
                    f"Optimizing MAP estimate. Iterations: {iter_+1} / {num_iter}.    "
                    f"Performance in iteration "
                    f"{divmod(iter_+1, save_best_every)[0] * save_best_every}: "
                    f"{best_log_prob.item():.2f} (= unnormalized log-prob)",
                    end="\r",
                )
                self.map_ = tf_inv(best_theta).detach()
        ...
        return tf_inv(best_theta).detach()
